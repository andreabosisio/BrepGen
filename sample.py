...
def sample(eval_args):
    ...
    # Classifier-free guidance -- https://sander.ai/2022/05/26/guidance.html 
    if eval_args['use_cf']:
        # Preparing input vector to compute both grad_x(log(p(x))) (uncond.) and grad_x(log(p(x|y))) (cond.)
    ...
    ### Models definition ###
    # Model that predicts the bounding box of the surface
    ...
    # Model that predicts the latents of the surface shape
    ...
    # Model that predicts the bounding box of the edge
    ...
    # Model that predicts the latents of the edge shape
    ...
    # Decoder of the latents of the surface shape generated by surfZ_model
    ...
    # Decoder of the latents of the edge shape generated by edgeZ_model
    ...
    with torch.no_grad():
        with torch.cuda.amp.autocast():
            ...
            for t in tqdm(pndm_scheduler.timesteps[:158], desc="PNDM Scheduler -- Surface position"):
                timesteps = t.reshape(-1).cuda()
                if class_label is not None:
                    _surfPos_ = surfPos.repeat(2,1,1)
                    pred = surfPos_model(_surfPos_, timesteps, class_label)
                    # Classifier-free guidance (combining uncond. and cond. predictions)
            ...
            for t in tqdm(ddpm_scheduler.timesteps[-250:], desc="DDPM Scheduler -- Surface position"):   
                timesteps = t.reshape(-1).cuda()
                if class_label is not None:
                    _surfPos_ = surfPos.repeat(2,1,1)
                    pred = surfPos_model(_surfPos_, timesteps, class_label)
            ...
            pndm_scheduler.set_timesteps(200)   
            for t in tqdm(pndm_scheduler.timesteps, desc="PNDM Scheduler -- Surface shape"):
                timesteps = t.reshape(-1).cuda()
                if class_label is not None:
            ...
            pndm_scheduler.set_timesteps(200)  
            for t in tqdm(pndm_scheduler.timesteps[:158], desc="PNDM Scheduler -- Edge position"): 
                timesteps = t.reshape(-1).cuda()   
                if class_label is not None:
                    _surfZ_ = surfZ.repeat(2,1,1)
                    _surfPos_ = surfPos.repeat(2,1,1)
            ...
            ddpm_scheduler.set_timesteps(1000)  
            for t in tqdm(ddpm_scheduler.timesteps[-250:], desc="DDPM Scheduler -- Edge position"):
                timesteps = t.reshape(-1).cuda()   
                if class_label is not None:
                    _surfZ_ = surfZ.repeat(2,1,1)
                    _surfPos_ = surfPos.repeat(2,1,1)
            ...
            ####################################################
            edgeMask = surfMask.unsqueeze(-1).repeat(1, 1, num_edges)
            ...
            for ii in range(batch_size):
                ...
                for surf_idx, bboxes in enumerate(edge_bboxs):
                    ...
                    for bbox_idx, bbox in enumerate(bboxes):
                        ...
                        same_rev = diff_rev < bbox_threshold
                        if same.sum()>=1 or same_rev.sum()>=1:
                            edgeMask[ii, surf_idx, bbox_idx] = True
                            continue # repeat value
                        else:
                            valid_bbox = np.concatenate([valid_bbox, bbox[np.newaxis,:,:]],0)
                    edgeMask[ii, surf_idx, 0] = False  # set first one to False  
            ...
            pndm_scheduler.set_timesteps(200)   
            for t in tqdm(pndm_scheduler.timesteps, desc="PNDM Scheduler -- Edge shape"):
                timesteps = t.reshape(-1).cuda()   
                if class_label is not None:
                    _surfZ_ = surfZ.repeat(2,1,1)
                    _surfPos_ = surfPos.repeat(2,1,1)
                    _edgePos_ = edgePos.repeat(2,1,1,1)
                    _edgeM_ = edgeMask.repeat(2,1,1)
                    _edgeZV_ = edgeZV.repeat(2,1,1,1)
                    noise_pred = edgeZ_model(_edgeZV_, timesteps, _edgePos_, _surfPos_, _surfZ_, _edgeM_, class_label)
                    noise_pred = noise_pred[:batch_size] * (1+w) - noise_pred[batch_size:] * w
                else:
                    noise_pred = edgeZ_model(edgeZV, timesteps, edgePos, surfPos, surfZ, edgeMask, class_label)
                edgeZV = pndm_scheduler.step(noise_pred, t, edgeZV).prev_sample

            edgeZV[edgeMask] = 0 # set removed data to 0
            edge_z = edgeZV[:,:,:,:12]
            ...
            edge_mask = edgeMask.detach().cpu().numpy()     
